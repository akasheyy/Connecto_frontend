  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setFollowers(Array.isArray(data?.user?.followers) ? data.user.followers : []);
      } catch (err) {
      ...
      <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <div style={styles.headerText}>
            ...
            <p style={styles.subtitle}>People following {user?.username}</p>
          </div>
        </div>
        ...
        <div style={styles.listArea}>
          {followers.length === 0 ? (
            <div style={styles.emptyState}>
              ...
              <p style={{ color: "#64748b" }}>No followers yet.</p>
            </div>
          ) : (
            followers.map((f) => {
...
const styles = {
  ...
  listArea: { padding: "15px", maxHeight: "70vh", overflowY: "auto" },
  userCard: {
    display: "flex",
    alignItems: "center",
    padding: "12px 16px",
